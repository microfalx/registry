...
final class DataImpl implements Data {
    ...
    public <T> T get() {
        ...
        if (isNotEmpty(valueAsString)) {
            ...
            try {
                ...
                return getSerde().to(valueAsString, (Class<T>) dataTypeClass);
    ...
    public <T> void set(T value) {
        if (value != null) {
            attributes.put(DATA_VALUE_ATTR, getSerde().asString(value));
    ...
    private Serde getSerde() {
        if (serde == null) {
            if (node != null) {
                serde = node.getRegistry().getSerde();
            } else {
                // only used for testing, so create a new instance
                serde = new JsonSerde();
                ((JsonSerde) serde).initialize();
            }
        }
        return serde;
    }
